...
export default function Lancto({ id, idLote, tipo, descricao, parcelas, parcela, valor, desconto, acrescimo, dataVencto, color, bgColor, bdColor, editColor, bgEditColor, delColor, bgDelColor, onEdit, onDelete }: LanctoProps) {
    const onEditClick = () => {
        if (onEdit) {
            onEdit(id);
        }
    ...
    return (
        <div className={styles.container} style={{
            ...
            <div className={styles.actionsBtn}>
                {
                    valor > 0 &&
                    <div className={styles.valor}><span>{valor}</span></div>
                }
                {
                    !valor && onDelete &&
                    <div >
                        <button type='button' className={`${styles.buttonOpt}`} style={{ backgroundColor: bgDelColor ?? 'inherited' }} onClick={onDeleteClick} data-toggle='tooltip' data-placement='right' title='Excluir'>
                            <BsFillTrashFill style={{ color: delColor ?? 'inherited', fontSize: '1.3rem' }} />
                        </button>
                    </div>
                }
                {
                ...
                }
                {
                    ...
                    <div>
                        <button type='button' className={`${styles.buttonOpt}`} style={{ backgroundColor: bgEditColor ?? 'inherited' }} onClick={onEditClick} data-toggle='tooltip' data-placement='right' title='Editar'>
                            <BsPencilFill style={{ color: editColor ?? 'inherited', fontSize: '1.3rem' }} />
                        </button>
                    </div>
                }
                {
